...
function AllTwists(C, K : CheckAutomorphisms := true)
	...
	if CheckAutomorphisms then
		...
		assert(#A eq #Abar);
		vprint User1 : "Automorphism group checked";
	end if;
	
	// Find H1
	M := map< G->Maps(A,A) | g:-> map<A->A | a:->[b : b in A | ApplyGalois(phi(a),rho(g)) eq phi(b)][1] > >;
	M := MapToAssociativeArray(M);
	vprint User1 : "Galois action on hom-set defined";
	H1CK := H1(G, A, M);
	vprint User1 : "H1 found";
	...
	vprint User1 : "Twists found";
	
	return L;
end function;

R<x> := PolynomialRing(Rationals());
C := SimplifiedModel(HyperellipticCurve(R![0, 1, 2, -3, -7], R![1, 1, 0, 1]));
K := QuadraticField(5);
AllTwists(C, K);
C2 := HyperellipticCurve(R![-2, -4, -2, 0, 1, 1], R![0, 0, 0, 1]);
K2 := NumberField(x^8 - 2*x^7 - 3*x^6 + 8*x^5 + x^4 - 2*x^3 + 23*x^2 - 12*x + 2);
AllTwists(C2, K2);
